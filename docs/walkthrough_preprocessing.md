...
> Feature engineering, preprocessing, pipeline persistence, and supporting backend checks.
...
| Status | File | Purpose |
|---|---|---|
| **Modified** | [`ui/window_data.py`](../ui/window_data.py) | Scrollable Data Engineering Lab with ingestion, cleaning, feature, and preprocessing controls |
...
The backend includes a helper to identify features that have a suspiciously high correlation (`> 0.95`) with the target. These features often "leak" future information and should be removed to ensure model generalization.
